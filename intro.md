...
# DBSHUIRI is a tool for quickly repairing damaged ORACLE files. It can address various corruption scenarios and swiftly fix ORACLE data files, enabling ORACLE to start up.
...
## If the current ORACLE redo log is damaged and cannot start, you can use the dsr ckpscn <scn> users01.dbf command to repair the checkpoint SCN in the datafile header to the specified SCN, and clear the FUZZY state, allowing ORACLE to start up.


### Use dsr hdr <datafile> to view the kcvfh datafile header information.
...
## If the ORACLE datafile header is damaged, for example, due to computer virus corruption or encryption, you can use the following series of commands to forge a valid datafile header, allowing ORACLE to start up.
...
&nbsp;
## In the case of a damaged control file, you can obtain the SQL statement to rebuild the control file by analyzing the SYSTEM01.DBF file. For example:
...
The above analysis of SYSTEM01.DBF was done using the dumpsys command, generating SQL statements for rebuilding the control file and DSR commands for forging all datafile headers.


## For damaged ORACLE EXP or EXPDP export files, you can use the dsr dumpexp or dsr dumppump commands to analyze the export files and generate SQL statements for the export files. For example:
...
## For damaged file systems or ASM, or accidentally deleted ORACLE data files, you can use the dsr scandev <device> command to scan the device. This works on the principle of merging data block fragments to recover the data files. For example:
...
## DSR can perform simple checks on data files, for example.
...
Unlike ORACLE's built-in DBV tool, even if the data file header is damaged, you can still use dsr dbv to check the remaining data blocks.


## Some auxiliary functions:

### dsr guess <datafile>: Guess the block size and ENDIAN of the data file
...
### dsr copy <source.dbf> <target.dbf> 10: Copy the 10th data block from source.dbf to the 10th data block in target.dbf

&nbsp;
&nbsp;
### dsr backuphdr users01.dbf : Backup the data file header
&nbsp;
&nbsp;
...
&nbsp;
### dsr restorehdr <backupfile> <target.dbf>: Restore the data file header from a backup
&nbsp;
...
&nbsp;
&nbsp;
### dsr parserdba 0x00400001: Parse RDBA to RFILE# BLOCK#
&nbsp;
&nbsp;
...
&nbsp;
&nbsp;
### dsr makerdba 5,1   make a RDBA from RFILE# BLOCK#
&nbsp;
&nbsp;
...
&nbsp;
&nbsp;
### dsr hxdb o1_mf_system_ld9rh3yb_.dbf 100  : Hexdump the 100th data block
&nbsp;
&nbsp;
...
&nbsp;
&nbsp;
### dsr parsenum 0xc202  : Parse a number from HEX
&nbsp;
&nbsp;
